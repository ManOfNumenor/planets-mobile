...
function setupSun(sunConfig) {
    for(const key of Object.keys(sun)) {
        if(sunConfig.hasOwnProperty(key)) {
            sun[key] = sunConfig[key];
        }
    }
...
function drawSun() {
    ...
    // old sun image-loading code, from before sun images were 
    // implemented in image-loading.js
    // if (!sun.image) {
    ...
    for (let sunNum = 0; sunNum<sunCount; sunNum++) {
        ...
        let imgScale = ((sun.radius*2)/(sun.imageVar.width*(2/3)))*scaleFactor;
        ...
        // console.log("drawing the sun scaled:"+imgScale+" which is:"+(sun.imageVar.width*imgScale)+"px wide");
        drawBitmapCenteredWithRotationAndScale(sun.imageVar,sun.x+ofsx,sun.y+ofsy,sunRotation,imgScale)
        // 2nd layer moves at a diff speed so the "shine lines" overlap and "glitter"
        drawBitmapCenteredWithRotationAndScale(sun.imageVar,sun.x+ofsx,sun.y+ofsy,-sunRotation*.666,imgScale)
        if (sun.sunspotsEnabled) drawSunspots(sun.x+ofsx,sun.y+ofsy,sun.radius*scaleFactor*extraScale,SUNSPOT_SCROLL_SPEED);
    }
}
